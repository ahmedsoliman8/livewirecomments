<div>
    ...
    <div class="flex">
        <div class="flex-shrink-0 mr-4">
            <img class="h-10 w-10 rounded-full" src="{{$comment->user->avatar()}}"
        ...
        <div class="flex-grow">
                ...
                @if($isEditing)
                    <form wire:submit.prevent="editComment">
                        <div>
                            ...
                            <textarea id="comment" name="comment" rows="3" wire:model.defer="editState.body"
            ...
            <div class="mt-2 space-x-2">
                ...
                @auth
                    @if($comment->isParent())
                        <button type="button" wire:click="$toggle('isReplying')" class="text-gray-900 font-medium">
                            Reply
                        </button>
                    @endif
                    @can('update',$comment)
                        <button type="button" wire:click="$toggle('isEditing')" class="text-gray-900 font-medium">
                            Edit
                        </button>
                    @endcan
                    @can('destroy',$comment)
        ...
        @if($isReplying)
            <form wire:submit.prevent="postReply" class="my-4">
                <div>
                    ...
                    <textarea id="comment" name="comment" rows="3" wire:model.defer="replyState.body"
                              class="shadow-sm block w-full focus:ring-blue-500 focus:border-blue-500 border-gray-300 rounded-md
